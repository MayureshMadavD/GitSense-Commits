...
*   **/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/customizeUI/mobilePage.tsx**
    ...
    *   Minor textual changes in the preview section's status messages (e.g., changing "Mobile" to "✔ Mobile"). No significant structural or logical changes apparent from the diffs.
...
## 23:01:27
The updates primarily focus on enhancing checkout dashboard UI customization, extending shipping rule capabilities, and introducing new bulk discount and upsell features, alongside continuous development and refinement of API integrations and client-side validations.

**File-specific Updates:**

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/customizeUI/loginPage.tsx`**
    *   **Consent Section Enhancement (29/10/2025, 18:44:05):** A new "Auto-select offers and updates consent box on checkout" switch (`is_consent_auto_select`) was added to the login page customization, allowing merchants to pre-select a consent box.
    *   **UI Refinement & Defaults (29/10/2025, 18:45:22 - 18:46:01):** Styling was adjusted for better visual separation in the consent section, and the new auto-select consent option was set to `true` by default.
    *   **Validation Improvement (29/10/2025, 18:58:39):** Logic was added to prevent saving an empty primary button text if the "Continue Button" is enabled.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/customizeUI/mobilePage.tsx`**
    *   **Non-substantive Changes (29/10/2025, 18:48:04 to 19:03:58):** Multiple entries show no functional changes, indicating minor saves or formatting adjustments.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/components/integrations/formRender.tsx`**
    *   **Loyalty Program Configuration (29/10/2025, 21:03:41):** This file defines a `RenderForm` component that includes extensive fields for configuring loyalty points, such as allowing COD with loyalty, stacking discounts, auto-application, custom display names, showing earned points, and defining calculation rules and sales channel control.

*   **`/Users/madav/Documents/dashboard_checkout/dashboard-ui-configuration/src/index.ejs`**
    *   **Environment-Specific URL Updates (30/10/2025, 07:53:01 - 31/10/2025, 23:02:32):** The URL for `@gokwik/utilities` in the local environment's import map was frequently adjusted, alternating between `qa-dashboard` and `sandbox-dashboard` domains, and eventually settling on `qa.gokwik.in`.
    *   **New Module Integration (30/10/2025, 07:53:01):** The `@gokwik/kwikcomm` module was added to the development and QA environment import maps.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/components/shipping/shippingBasedData.tsx`**
    *   **Shipping ID Selection Liberalization (31/10/2025, 23:05:43):** A previous restriction on disabling certain shipping IDs from being selected in the UI was removed, allowing more flexible mapping.
    *   **Non-substantive Changes (30/10/2025, 10:02:10 to 10:34:46 & 31/10/2025, 12:10:00 to 12:16:10):** Several entries show identical code, indicating minor saves or formatting changes.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/shipping-settings/productBasaedShipping.tsx`**
    *   **Multiple Rule Support (31/10/2025, 23:09:53):** Logic was updated to allow multiple product-based shipping rules per shipping ID.
    *   **Deletion Logic Refinement (31/10/2025, 23:40:49 - 01/11/2025, 12:01:32):** The rule deletion logic was iteratively refined to handle scenarios with single or multiple rules, including adding `.trim()` for robust name comparisons, though the underlying deletion method (`delete` on array elements) is suboptimal.
    *   **Debugging Phases (30/10/2025, 10:36:15 & 31/10/2025, 23:19:31 - 23:26:00):** Multiple entries show debugging efforts, with `console.log` statements and temporary commenting/uncommenting of save operations.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/shipping-settings/shippingBasedService.tsx`**
    *   **Product Based Shipping Tab (30/10/2025, 10:49:18):** A new "Product Based Shipping" tab was added, visible conditionally based on a `merchantConfig` setting.
    *   **Minor Artifact (05/11/2025, 20:01:43):** An empty item was introduced into the `tabsList` array.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/shipping-settings/shippingBasedMethods.tsx`**
    *   **Non-substantive Changes (30/10/2025, 10:53:28 to 11:00:27):** Multiple entries show no functional changes, indicating minor saves or formatting adjustments.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/bulk-discounts/createBulkDiscounts.tsx`**
    *   **AWS Upload Header Toggles (30/10/2025, 18:19:56 & 18:30:51):** The `includeUserIdHeader` parameter for S3 file uploads was repeatedly toggled between `true` and `false`.
    *   **Temporary Upload Disablement (30/10/2025, 18:25:19 & 18:25:34):** The AWS S3 upload logic was temporarily commented out for testing, with actual upload re-enabled at 18:26:49.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/store/discounts/api.ts`**
    *   **Discount & Shopify Data Utilities (30/10/2025, 18:29:20):** This file provides utilities for customer eligibility, discount code matching, Shopify product/collection data mapping, and preparing API payloads for various discount types.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/free-gift/freeGift.tsx`**
    *   **Free Gift Rule Configuration (03/11/2025, 11:41:24):** A new component was introduced to configure free gift rules based on various criteria (e.g., cart value, quantity, specific products/collections), including selection of free gift products, quantity, price, and application methods.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/checkout-configs/tieredDiscounts.tsx`**
    *   **Tiered Discount Management (03/11/2025, 16:16:44):** A new component was added to manage tiered prepaid discounts, supporting "Non Freebie" (value-based) and "Freebie" (product-based) types, with options for limits, values, payment methods (excluding COD), and inline editing.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/utilities/helpers/validations.ts`**
    *   **Comprehensive Validation Logic (03/11/2025, 16:19:20):** This file centralizes validation functions for tiered discounts (checking for unique codes, non-overlapping ranges, and specific freebie rules), dynamic discounts, "Buy X at Y" discounts, discount capping, and Gift with Purchase (GWP). A new rule prevents different freebie products for tiers with common ranges.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/utilities/api.ts`**
    *   **New Upsell Data API (05/11/2025, 22:29:02):** A new function `getAndDeleteUpsellData` was added to fetch or delete upsell configuration data, indicating the introduction of upsell management features.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/utilities/constants/apiEndpoints.ts`**
    *   **New Upsell Endpoints (05/11/2025, 22:28:25):** New API endpoints for "Cart Upsell" were added (`/api/dashboard/cart-upsell` and `/api/dashboard/cart-upsell/upsert`), signifying backend support for the new upsell feature.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/components/upsell/upsellFormRenderer.tsx`**
    *   **Dynamic Upsell Rule Form (05/11/2025, 22:31:23):** This component was introduced to render forms for individual upsell rules, including title, item count, product/collection applicability, upsell items, and optional discounted pricing.
    *   **`useEffect` Dependency Flux (05/11/2025, 22:31:50 - 23:00:14):** The `useEffect` dependencies saw frequent changes, reflecting an iterative process of debugging and optimizing how form data is initialized and reacts to external changes.
    *   **Conditional Default Values (05/11/2025, 22:48:40 - 22:49:20):** Logic was refined to set default values for radio buttons and `discounted_price` only if they are not already explicitly set.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/components/upsell/upsellCheckout.tsx`**
    *   **Upsell Management Interface (05/11/2025, 22:45:26):** This new component allows merchants to configure cart upsell rules with dynamic form generation, fetching existing data, and adding/deleting rules.
    *   **UI Structure (05/11/2025, 22:57:37 - 22:59:19):** The `upsellItems` are now managed via `useState` and a helper function, allowing for dynamic updates. The `Collapse` component was adjusted to allow multiple panels to be open and destroy inactive ones.
    *   **Initialization Debugging (05/11/2025, 22:58:37):** Initial values for new upsell rules were toggled between `null` and empty objects/arrays during this period, indicating active debugging of initial state.

**Recurring Elements and Patterns:**

*   **`gokwik-ui-kit` Usage:** The UI components (`Button`, `Form`, `Card`, `Switch`, `Input`, `Table`, `Drawer`, `Collapse`, `Radio`, `Checkbox`, `InputNumber`, `DatePicker`, `Select`, `Tooltip`, `message`) are consistently sourced from `gokwik-ui-kit`, indicating a standardized component library.
*   **State Management with Hooks:** `useState` and `useEffect` are extensively used across components for local state management and side effects.
*   **`useMerchantConfigApi` Hook:** This custom hook is frequently used for fetching, caching, and saving merchant configuration data, suggesting a centralized approach to configuration management.
*   **Form-Driven UI:** Components like `loginPage.tsx`, `formRender.tsx`, `freeGift.tsx`, `tieredDiscounts.tsx`, and `upsellFormRenderer.tsx` heavily rely on `Form` and `Form.Item` for defining structured input fields and their validation rules.
*   **API Interaction Pattern:** `makeAPICall` is the standard utility for all API requests, often used in conjunction with `APIEndPoints` for clear endpoint definitions.
*   **Product/Collection Selection:** The `ProductSearch` component is a recurring element for selecting specific products or collections, used in `shippingBasedData.tsx`, `freeGift.tsx`, and `upsellFormRenderer.tsx`.
*   **Iterative Development/Debugging:** Multiple consecutive log entries for the same file, with minor changes (e.g., adding `console.log`, commenting out code, or tweaking `useEffect` dependencies), suggest an ongoing, iterative development and debugging process. This is prominent in shipping, bulk discounts, and upsell features.
*   **Configuration Flexibility:** Many features (e.g., consent auto-select, product-based shipping, upsells) include switches or radio buttons to enable/disable or modify their behavior, indicating a focus on merchant configurability.
*   **Detailed Validations:** Dedicated validation functions are implemented to ensure data integrity and user input correctness before submitting configurations.