...
## 22:30:21
Changes span multiple files related to checkout dashboard UI, shipping settings, discount management, and UI customization.

### File-Specific Updates:

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/shipping-settings/productBasaedShipping.tsx`**
    *   **Timestamp Range:** 29/10/2025, 17:23:01 to 01/11/2025, 12:04:22
    *   **Summary of Changes:** This component manages product-based shipping rules. Initial changes involved setting up the form, table, and drawer for adding/editing rules, including validation for product/collection selection and setting default boolean values. A significant ongoing change throughout the logs revolves around refining the `handleFormSubmit` logic, particularly how `product_based_rules` are handled during edits. Initially, it appended new rules, then introduced complex, and at times erroneous, logic for deleting existing rules before or during an update, specifically when a drawer is open and a `previousShippingId` is active. This deletion logic saw several iterations, attempting to handle cases where there's one rule or multiple rules, and trying to target specific rules by name. Debugging `console.log` statements were frequently added and removed, and the `saveConfig` call was often commented out/in, indicating active development and testing of the submission flow and rule manipulation.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/components/shipping/shippingBasedData.tsx`**
    *   **Timestamp Range:** 29/10/2025, 17:29:02 to 01/11/2025, 12:16:10
    *   **Summary of Changes:** A new utility function `processProductBasedShippingData` was introduced. This function handles the core logic for processing and presenting product-based shipping data. It includes mechanisms for:
        *   Flattening shipping rules for table display (`tableData`).
        *   Mapping product/collection data for UI rendering (`dataMapperForItems`).
        *   Generating dynamically enabled/disabled options for selecting shipping IDs (`selectedOptions`). The `disabled` logic for options was commented out in a later change (31/10/2025, 23:05:43), potentially to allow all options to be selectable regardless of current usage.
        *   Defining table columns for displaying selected items (`itemColumnData`), including functionality to remove items and update form fields.
        *   Defining an array of form fields (`fields`) for rule creation/editing, incorporating `ProductSearch` components to manage associated products and collections.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/store/customize-ui/index.ts`**
    *   **Timestamp Range:** 29/10/2025, 18:04:43 to 29/10/2025, 18:27:17
    *   **Summary of Changes:** This Redux slice received updates to its `initialState.login_data`. A typo `is_ isContinueButtonVisible` was corrected to `isContinueButtonVisible`, a new property `is_consent_auto_select: true` was added, and `is_consent_required` was renamed to `isConsentVisible`. These changes reflect evolving requirements for the login page's UI customization options.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/customizeUI/loginPage.tsx`**
    *   **Timestamp Range:** 29/10/2025, 18:09:05 to 29/10/2025, 18:58:39
    *   **Summary of Changes:** This component handles customization for the checkout login page. Key changes focused on refactoring UI elements into a dynamic `fields` array rendered by `RenderField`. The "Primary Button" and "Consent Section" UI configurations were moved into this array, allowing for a more modular form structure. Specific updates include:
        *   Adding a description directly into the "Primary Button" field's label.
        *   Refining CSS classes for text (`font-light`, `font-normal`).
        *   Renaming `is_consent_required` to `isConsentVisible` and adapting related logic and `initialValues`.
        *   Replacing the generic `RenderEditorFunction` for the Consent Section with a custom rendering that includes a toggle (`isConsentVisible`) and a new `BannerComponentRendrer` for the actual content.
        *   Introducing an `is_consent_auto_select` switch within the Consent Section.
        *   Adding form validation to ensure the "Primary Button" CTA text is not empty if the button is visible during form submission.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/customizeUI/mobilePage.tsx`**
    *   **Timestamp Range:** 29/10/2025, 18:48:04 to 29/10/2025, 19:03:58
    *   **Summary of Changes:** This component provides a mobile preview of the customized UI. No significant code changes were observed in the provided log entries for this file, indicating stability or minor re-saves across the timestamps.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/components/integrations/formRender.tsx`**
    *   **Timestamp:** 29/10/2025, 21:03:41
    *   **Summary of Changes:** This file introduced a generic `RenderForm` component. It's designed to dynamically render various input types for integration settings, including switches, selects, number inputs, etc. It incorporates tooltips and alerts for user guidance on features like loyalty points, EDD, and pincode serviceability, and includes conditional rendering based on form values.

*   **`/Users/madav/Documents/dashboard_checkout/dashboard-ui-configuration/src/index.ejs`**
    *   **Timestamp Range:** 30/10/2025, 07:53:01 to 31/10/2025, 23:03:18
    *   **Summary of Changes:** This configuration file manages the application's microfrontend imports via SystemJS import maps. The primary change involved updating the URLs for several `@gokwik` packages, particularly for the `isLocal` environment. Initially, most were linked to `qa-dashboard.dev.gokwik.in`. A later change (30/10/2025, 16:50:46) shifted `@gokwik/utilities`, `@gokwik/dashboard`, `@gokwik/checkout-platform` to `sandbox-dashboard.dev.gokwik.in`. Finally, `@gokwik/utilities` was further updated to `https://qa.gokwik.in/utilities/gokwik-utilities.js` (31/10/2025, 23:02:32). These updates reflect changes in where these microfrontends are deployed or served during local development and testing.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/shipping-settings/shippingBasedService.tsx`**
    *   **Timestamp Range:** 30/10/2025, 10:46:02 to 30/10/2025, 10:49:18
    *   **Summary of Changes:** This component defines the tab structure for shipping settings. The "Product Based Shipping" tab was modified to be conditionally rendered, appearing only if `merchantConfig?.config?.custom_config?.shipping_service` is enabled.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/shipping-settings/shippingBasedMethods.tsx`**
    *   **Timestamp Range:** 30/10/2025, 10:53:28 to 30/10/2025, 11:00:27
    *   **Summary of Changes:** This component configures individual shipping methods, including their name, price, and order value limits, along with advanced conditions. It handles CSV uploads for pincode lists. No significant code changes were observed across the listed timestamps, indicating minor edits or re-saves during that period.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/bulk-discounts/createBulkDiscounts.tsx`**
    *   **Timestamp Range:** 30/10/2025, 17:52:43 to 30/10/2025, 18:30:51
    *   **Summary of Changes:** This component facilitates the creation of bulk discount codes, allowing for random generation or CSV upload. Key changes involved alternating between commenting and uncommenting the AWS S3 `makeAWSPutCall` function and associated logic for CSV file uploads (30/10/2025, 18:19:56, 18:25:19, 18:26:49). This suggests active testing or debugging of the file upload mechanism for bulk discounts. It also defines validations for discount set titles, expiry dates, and the feasibility of random code generation.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/store/discounts/api.ts`**
    *   **Timestamp:** 30/10/2025, 18:29:20
    *   **Summary of Changes:** This file introduced several utility functions to standardize the mapping and processing of discount-related data for API interactions. This includes logic for customer eligibility, assigning match types, formatting Shopify IDs, and fetching/mapping product and collection data. The `requestMapDiscount.setup` function was added to construct comprehensive API payloads for various discount types, managing updates and deletions for existing discounts.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/free-gift/freeGift.tsx`**
    *   **Timestamp:** 03/11/2025, 11:41:24
    *   **Summary of Changes:** This new component, `FreeGift`, was introduced to configure automatic free gift rules. It provides a form for defining rule details like name, sales channels, purchase requirements (cart value/quantity), product/collection applicability, free gift quantity and price, and application method (auto/selection). It also features a table to display existing rules, allowing users to enable/disable, edit, or delete them.

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/pages/settings/checkout-configs/tieredDiscounts.tsx`**
    *   **Timestamp:** 03/11/2025, 16:16:44
    *   **Summary of Changes:** This new component, `TieredDiscounts`, was added to manage tiered discount rules. It uses `useMerchantConfigApi` to fetch and save discount configurations. It supports different payment methods and discount types (e.g., "Freebie" vs. "Non Freebie"), allowing for definition of lower/upper order limits, discount values, and capping amounts. It provides an editable table interface for adding, editing, and deleting tiered discounts, with specific handling for "Freebie" discounts (no value, includes `is_driven_by_rto_actions`).

*   **`/Users/madav/Documents/dashboard_checkout/checkout-dashboard-ui/src/library/utilities/helpers/validations.ts`**
    *   **Timestamp:** 03/11/2025, 16:19:20
    *   **Summary of Changes:** This file received updates to its validation functions. The `validateTieredDiscount` function was significantly enhanced to perform comprehensive checks, including:
        *   Ensuring unique discount codes per payment method.
        *   Preventing overlapping discount ranges and invalid limit values.
        *   Restricting only one discount with an empty upper limit.
        *   Validating the presence of discount values for "Non Freebie" and free gifts for "Freebie" types.
        *   A new crucial validation was added to prevent "different freebie products for tiers with common ranges."
        *   The file also contains validations for dynamic discounts, BuyXatY discounts, and discount capping, focusing on preventing duplicates and ensuring logical consistency in ranges and values.

### Patterns and Recurring Elements:

*   **Timestamp Format:** All timestamps consistently use the "DD/MM/YYYY, HH:MM:SS" format.
*   **React-centric Development:** The majority of changes are within React functional components (`.tsx` files), heavily utilizing React hooks (`useState`, `useEffect`, `useMemo`, `useRef`) for local state management and lifecycle hooks.
*   **Gokwik UI Kit:** There's a strong and consistent reliance on the `gokwik-ui-kit` for UI components (e.g., `Form`, `Button`, `Card`, `Table`, `Drawer`, `Input`, `Select`, `Switch`, `Tooltip`, `message`).
*   **Form Management with Ant Design-like Structure:** The `Form` component is central to data input, featuring `Form.useForm()`, `Form.useWatch()`, and structured fields arrays passed to a generic `RenderField` component for rendering, along with extensive `rules` for validation.
*   **API Interactions and Configuration:** `useMerchantConfigApi` is a recurring hook for fetching and saving merchant configuration data. Direct API calls using `makeAPICall` and `makeAWSPutCall` (from `@gokwik/utilities`) are used, often referencing environment variables (`process.env.REACT_APP_BASE_URL`).
*   **Redux for Global State:** `useSelector` and `useAppDispatch` are frequently employed for accessing and modifying global state managed by Redux slices (e.g., `customize-ui`, `discounts`, `user`).
*   **Conditional Logic and Rendering:** Extensive use of conditional statements (`if`, `? :`, `&&`) for dynamically showing or hiding UI elements or executing logic based on component state or configuration properties (e.g., `activeTab`, `open` drawer, `isBannerEnabled`, `codeGenerationMethod`).
*   **Debugging Practices:** The intermittent commenting out of `saveConfig` calls and insertion of `console.log` statements in `productBasaedShipping.tsx` is a notable pattern, indicating active debugging during development.
*   **Data Transformation and Validation:** Multiple helper functions (`dataMapperForItems`, `mapCollections`, `mapProducts`, `validateTieredDiscount`, `rangesOverlap`) are dedicated to transforming data formats for UI or API consumption and ensuring data integrity before persistence.
*   **Microfrontend Architecture:** The `index.ejs` file consistently demonstrates a Single-SPA microfrontend architecture by dynamically importing various `@gokwik` packages from different URLs based on the deployment environment (`local`, `dev`, `qa`, `sandbox`, `production`).
*   **Focus on Shipping and Discounts:** A significant portion of the changes centers around configuring and managing shipping rules (product-based, standard methods) and various discount types (bulk, tiered, free gift), including their UI, validation, and backend integration.