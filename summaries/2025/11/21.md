...
## 23:09:13
The changes primarily involve updates to UI components for a checkout dashboard, focusing on customization, order details, free gift management, and shipping rules. There are also environmental configuration changes and updates to a core templating file.

**File-specific updates:**

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/customizeUI/brandPage.tsx` (Timestamp: 16/11/2025, 13:10:34)**
    *   This file manages the "Brand" section of the UI customization, allowing merchants to upload logos and set primary colors.
    *   It imports various UI components (Button, Upload, Form, ColorPicker, Card, Alert) from `gokwik-ui-kit`.
    *   It interacts with Redux for merchant details and dispatches actions to update logo and brand data.
    *   API calls are made to `downloadLogo`, `presingedLogoUrl`, and `updateMerchant` for logo management.
    *   The `handleUpload` function cleans and uploads images to AWS S3.
    *   A `ColorPicketComponent` is used for theme color selection.
    *   The form uses `Form.useWatch` to react to changes in `brand_data` and enables/disables the "Save Changes" button based on `isVisible` state.

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/discounts/productSearch.tsx` (Timestamp: 17/11/2025, 16:22:51 & 21/11/2025, 11:48:56)**
    *   This component provides a search and selection interface for Shopify products or collections, typically used for discount or gift configurations.
    *   It uses `useState`, `useEffect`, `useRef`, and `useIntersectionObserver` hooks for managing search state, pagination, and infinite scrolling.
    *   It fetches data using `fetchShopifyCollectionsOrProducts` and parses responses for display.
    *   It handles adding/removing selected items, quantity changes, and specific gift pricing (`handleGiftPrice`).
    *   **Significant change on 21/11/2025:** Added a new prop `btnSearch` to control the visibility of a "Select products/collections" button, and `DeleteOutlined` icon was imported (though not explicitly used in the provided snippet). The display logic for `product?.variants?.length` was also refined.

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/orders/order-details/index.tsx` (Timestamp: 17/11/2025, 16:23:04)**
    *   This page displays detailed information about an order.
    *   It imports a wide array of UI components (Button, Col, Row, Divider, Table, Tabs, Tag, Tooltip, Popover, message) from `gokwik-ui-kit`.
    *   It manages order data, refund details (`RefundDrawer`), and customer contact information (masked by default).
    *   It includes logic for merging repeated refunds and calculating refundable amounts.
    *   The `formatCurrency` and `formatAmountWithCurrency` helpers are used for consistent currency display.
    *   It determines whether RTO actions and scores should be shown based on merchant configuration.
    *   It dynamically sets an edit title and disables editing based on applied rewards, GWP discounts, or loyalty points.

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/free-gift/freeGift.tsx` (Timestamps: 18/11/2025, 00:49:40, 00:50:14, 00:58:39)**
    *   This page allows configuring free gift rules for merchants.
    *   It utilizes `ProductSearch` component for selecting free gift products and collections.
    *   It uses a form with `RenderField` for various input types (input, checkbox, radio, inputNumber, search, switch).
    *   Rules can be defined based on sales channels, purchase type (cart value, quantity), and applicability (all products, specific products/collections).
    *   It manages a list of free gift rules with options to enable/disable, edit, and delete them, displayed in a `Table` with `Switch` and `Dropdown` for actions.
    *   Validation is included for minimum/maximum values and ensuring at least one sales channel is selected.
    *   The `handleAddFreeGiftProduct` and `handleAddApplicableOnItems` functions manage the addition of products/collections, with checks to prevent duplicate additions.
    *   **Pattern:** Multiple commits on the same day with minor changes, suggesting active development and refinement of this feature. The content for these three commits is identical, implying no functional change was introduced but possibly formatting or metadata updates.

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/orderConfirmationPage/collapseFields.tsx` (Timestamp: 18/11/2025, 23:42:52)**
    *   This component defines collapsible sections for customizing the order confirmation UI.
    *   It includes sections for "Brand Logo Customization," "Dynamic Banners," "TrustBanner," "Order Details," "Customer Information," and "Support & Communication."
    *   Each section contains `Form.Item` fields for toggling visibility (`Switch`) and various input types (input, custom renders for `ColorPickerHelper`, `RenderEditorFunction`, `TrustBadgeForm`).
    *   It also features drag-and-drop reordering functionality for the collapse items.

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/orderConfirmation/index.tsx` (Timestamps: 19/11/2025, 21:10:01, 21:11:54, 21:12:22, 21:12:29)**
    *   This page provides the main interface for customizing the order confirmation page.
    *   It uses `InfoComponent`, `MainSection`, `RenderField`, and includes `Alert` for guidance.
    *   A form is used to manage settings, with a "Save Changes" button enabled when `changed` state is true.
    *   Validation (`form.validateFields`) is implemented, with specific fields excluded from validation.
    *   The layout is a two-column design with settings on the left and a `OrderConfirmationPreview` on the right.
    *   **Pattern:** Multiple commits on the same day with very minor layout adjustments (`Col flex` property changed from `1 1 200px` to `1 1 300px` and back), indicating fine-tuning of the UI responsiveness.

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/shipping/shippingBasedData.tsx` (Timestamps: 20/11/2025, 18:44:34, 18:44:55, 18:49:44, 18:49:50)**
    *   This component manages product-based shipping rules.
    *   It uses `ProductSearch` to add products or collections to shipping rules.
    *   It displays existing rules in a `Table` with options to edit and delete.
    *   Rules include name, price, mapped shipping ID, calculation method (override/add onto base), price multiplier, and combination with weight-based shipping.
    *   Input validation prevents adding similar product variants or collections multiple times.
    *   **Significant change on 20/11/2025, 18:49:44 (and 18:49:50):** The `price` input type changed from `inputNumber` to `input` with a `onKeyPress` validator to only allow numeric and dot characters, likely to handle decimal inputs more flexibly or to prevent non-numeric input altogether.
    *   **Pattern:** Multiple commits on the same day, suggesting active development and minor corrections/improvements to the shipping rule configuration. The last two commits are identical, possibly reflecting minor non-functional saves.

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/utilities/helpers/helper.ts` (Timestamps: 21/11/2025, 11:50:18, 11:51:40)**
    *   This utility file contains various helper functions for the UI.
    *   Includes functions for dynamic search filters, date formatting (`formatDateString`, `getFutureDateString`), currency formatting (`setCurrencySymbol`, `formatAmount`), Shopify app checkout status, deep object comparison (`deepCompare`), debouncing, disabled date/time for pickers, capitalizing text, number formatting, extracting table column info, sorting data, uploading CSV for cohorts, and comparing array fields.
    *   **Pattern:** Two commits very close in time with identical code, indicating a save without functional changes.

**General Patterns and Recurring Elements:**

*   **Checkout Dashboard UI Context:** A large portion of the changes are within the `checkout-dashboard-ui` directory, indicating ongoing development and refinement of merchant-facing customization and management features for a checkout experience.
*   **Form-based UI:** The widespread use of `Form` and `Form.Item` from `gokwik-ui-kit`, along with custom `RenderField` components, points to a heavily form-driven user interface for configuration and data entry.
*   **Merchant Configuration:** Many components (`BrandPage`, `FreeGift`, `OrderConfirmationPage`, `shippingBasedData`) interact with `useMerchantConfigApi` and `Form.useWatch(['merchant_config', ...])`, highlighting a focus on allowing merchants to deeply customize their checkout experience.
*   **Product/Collection Selection:** The `ProductSearch` component is a recurring element, enabling merchants to select specific products or collections for various rules (e.g., free gifts, shipping).
*   **Styling and Layout:** Extensive use of Tailwind CSS classes (`flex`, `gap-x`, `w-full`, `bg-gray-50`, `rounded-md`, `p-2`) for styling and responsive layouts.
*   **UI Toolkit:** The `gokwik-ui-kit` is consistently used for standard UI components like Buttons, Inputs, Tables, Switches, Dropdowns, etc.
*   **API Interactions:** Components frequently make API calls using `makeAPICall` and `makeAWSPutCall` to fetch and save configuration or order-related data.
*   **State Management:** `useState` and `useEffect` are fundamental for managing component-level state and side effects, often combined with Redux (`useAppDispatch`, `useSelector`) for global state.
*   **Timestamp Patterns:** Several files show multiple commits within a very short timeframe (e.g., `freeGift.tsx`, `orderConfirmation/index.tsx`, `shippingBasedData.tsx`, `helper.ts`). This often indicates rapid iteration, minor adjustments, or frequent saving during development.
*   **Environmental Configuration:** Updates to `index.ejs` and `.env` files consistently involve changing the base URLs for different deployment environments (`dev`, `qa`, `sandbox`, `production`). The `index.ejs` specifically manages import maps for different `@gokwik` micro-frontends depending on the `deployedEnv` variable. The `.env` file updates switch the active API endpoints between `qa` and `sandbox` environments.