...
## 16:35:06
The recent code changes primarily focus on enhancing and refining UI components and backend integrations across a checkout dashboard application, particularly in settings related to brand customization, discount management, order details, and shipping rules.

**File-Specific Updates:**

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/customizeUI/brandPage.tsx` (16/11/2025, 13:10:34):** This component was updated to manage brand UI settings. It includes functionality for logo upload with file type/size validation and AWS S3 integration, along with a color picker for theme customization. It uses Redux for state management and saves configuration via API calls.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/discounts/productSearch.tsx` (17/11/2025, 16:22:51 & 21/11/2025, 11:48:56):** The `ProductSearch` component, used for selecting products or collections in discount rules, received several updates. It was refined to handle collated products (grouping variants) and included logic for collection-specific quantity validation. A new `btnSearch` prop was introduced to offer an alternative "Select Products/Collections" button, and the display of variant information was improved.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/orders/order-details/index.tsx` (17/11/2025, 16:23:04):** This component for displaying order details was updated to include custom currency formatting, a `mergeRepeatedRefunds` utility for consolidating refund attempts, and logic to determine the order failure reason. It also dynamically sets editability based on applied rewards or discounts.
*   **`/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-configuration/src/index.ejs` (18/11/2025, 00:45:31; 20/11/2025, 18:37:48; 21/11/2025, 12:27:47; 24/11/2025, 12:17:06; 24/11/2025, 12:17:41):** This EJS template, central to the micro-frontend architecture, saw several changes to its `systemjs-importmap`. These changes primarily involved updating the base URLs for various `@gokwik/*` micro-frontends (e.g., `utilities`, `dashboard`, `checkout`, `payment`, `kwikpass`, `rto`, `kwikship`, `ellora`, `checkout-platform`, `shopify-app`, `kwikcomm`) across different deployment environments (local, dev, qa, sandbox). The `isLocal` condition's mappings were specifically adjusted multiple times, toggling between `sandbox-dashboard.dev.gokwik.in` and `qa-dashboard.dev.gokwik.in` for external micro-frontends.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/free-gift/freeGift.tsx` (18/11/2025, 00:49:40, 00:50:14, 00:58:39):** This component manages automatic free gift rules. It provides a form for defining rules based on sales channels, purchase type, value ranges, and applicable products/collections. It integrates with `ProductSearch` for item selection and displays rules in a table with edit/delete actions and an enable/disable switch.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/orderConfirmationPage/collapseFields.tsx` (18/11/2025, 23:42:52):** This component was introduced to organize UI customization options for the order confirmation page into draggable, collapsible sections (e.g., Brand Logo, Dynamic Banners, TrustBanner, Order Details, Customer Information, Support & Communication), each with an enable/disable switch and specific configuration fields including color pickers.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/orderConfirmation/index.tsx` (19/11/2025, 21:10:01, 21:11:54, 21:12:22, 21:12:29):** This page provides a main interface for configuring the order confirmation page, featuring an "Enable Custom Order Confirmation Page" toggle, an informational alert, and a split layout for form configuration and a live preview. UI layout adjustments were made to the column `flex` properties.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/shipping/shippingBasedData.tsx` (20/11/2025, 18:44:34, 18:44:55, 18:49:44, 18:49:50):** This component (likely a sub-component for shipping settings) handles product-based shipping rules. A notable change was the alteration of the `price` input type from a number input to a text input with a custom `onKeyPress` event to explicitly allow decimal numbers, indicating a more precise control over price entry. It includes logic for mapping items, validation against existing rules, and integration with `ProductSearch`.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/utilities/helpers/helper.ts` (21/11/2025, 11:50:18 & 11:51:40):** This utility file, though listed twice with identical content, contains various helper functions for date and amount formatting, deep object comparison, debouncing, CSV upload validation (specifically for phone numbers or emails), and data extraction/sorting for UI components.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/shipping-settings/shipping-component/shippingBasedMethods.tsx` (24/11/2025, 12:31:55 & 12:36:01):** This component configures general shipping methods. It was updated to handle CSV uploads for pincodes and customer cohorts, including fetching pre-signed URLs and uploading to AWS S3, with robust CSV content validation. It also includes form fields for shipping name, price, order value ranges, and integrates with customer segmentation.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/utilities/helpers/validations.ts` (Numerous timestamps between 24/11/2025, 12:45:40 and 24/11/2025, 16:34:08):** This file, containing various validation functions, was heavily touched, though most changes between timestamps appear to be minor formatting or very subtle logic adjustments within existing validation functions (e.g., `validateTieredDiscount`, `validateDynamicDiscount`, `validateBuyXatYDiscount`, `validateDiscountCapping`, `validateGWP`). A functional change observed was the addition of `Weight_based_Shipping_Rates` to its imports and updates to `validateWeightRangeDifference` with an `isEditEnabled` parameter.
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/shipping-settings/shipping-component/weightShipping.tsx` (Numerous timestamps between 24/11/2025, 15:16:01 and 24/11/2025, 16:32:57):** This component for weight-based shipping methods also saw frequent minor updates, especially in how it calls the `validateWeightRangeDifference` function, explicitly passing `false` for `isEditEnabled` in non-edit contexts. It provides form fields for defining weight ranges and prices, mapping to existing shipping IDs, and displays these rules in a filterable table.

**Timestamps of Significant Changes:**

*   **16/11/2025, 13:10:34:** Initial implementation or major update to the `BrandPage` for UI customization.
*   **17/11/2025, 16:22:51 - 16:23:04:** Updates to `ProductSearch` and `OrderDetails`, indicating work on discount selection and order visibility.
*   **18/11/2025, 00:45:31:** First recorded change to `index.ejs`, reflecting micro-frontend configuration adjustments.
*   **18/11/2025, 00:49:40 - 00:58:39:** Iterative updates to `freeGift.tsx`, likely feature development for free gift rules.
*   **18/11/2025, 23:42:52:** Introduction of `collapseFields.tsx` for structured UI customization of order confirmation.
*   **19/11/2025, 21:10:01 - 21:12:29:** Multiple small adjustments to `orderConfirmation/index.tsx`, mainly UI layout tweaks.
*   **20/11/2025, 18:37:48:** Significant change in `index.ejs` micro-frontend import map, switching `isLocal` environment configurations.
*   **20/11/2025, 18:49:44:** Functional change in `shippingBasedData.tsx` to allow decimal input for price.
*   **21/11/2025, 11:48:56:** Functional additions to `ProductSearch` component (e.g., `btnSearch` prop, conditional quantity application).
*   **21/11/2025, 12:27:47:** Reversion of `isLocal` environment settings in `index.ejs` from QA back to Sandbox.
*   **24/11/2025, 12:31:55:** Major update to `shippingBasedMethods.tsx` to support CSV uploads for pincodes/cohorts.
*   **24/11/2025, 15:16:08:** Functional change in `validations.ts` to import `Weight_based_Shipping_Rates` and parameter updates for validation functions.

**Patterns and Recurring Elements:**

*   **UI Framework & Components:** Consistent use of `gokwik-ui-kit` components (Buttons, Forms, Tables, Modals, etc.) across various dashboard pages for a unified UI experience.
*   **Form-centric Development:** A strong emphasis on form creation, validation, and submission for configuring various settings (brand, discounts, shipping). `Form.useForm()` and `Form.useWatch()` are heavily utilized.
*   **Data Validation:** Extensive use of client-side validation, often handled in a centralized `validations.ts` file, particularly for complex scenarios like overlapping ranges in discount tiers or shipping rules, and uniqueness constraints.
*   **Micro-frontend Architecture:** The presence of `index.ejs` with `systemjs-importmap` entries for different `@gokwik/*` services clearly indicates a micro-frontend approach, with frequent updates to these mappings for environment-specific deployments.
*   **API Interaction Patterns:** A consistent pattern of fetching data (e.g., merchant configs, customer segments, order details), processing it, and saving updates back to the backend via `makeAPICall` and `makeAWSPutCall`.
*   **Helper Utilities:** Regular use of shared helper functions (e.g., `formatAmount`, `uploadCohortCsv`) for common tasks, promoting code reusability.
*   **Iterative Development/Refinement:** Numerous timestamps for the same file, especially `freeGift.tsx`, `orderConfirmation/index.tsx`, `shippingBasedData.tsx`, `weightShipping.tsx`, and `validations.ts`, suggest an iterative development process involving frequent small changes, formatting adjustments, or minor bug fixes.
...
*   **Initial State (22/11/2025, 17:38:55):**
    *   The file defines `APIRequestConfig` interface for API calls and `EventsInterface` for user events.
    *   It exports an `async function makeAPICall` which handles Axios-based API requests, including:
        *   Restricting internal users from non-GET operations on certain URLs.
        *   Setting up Axios instance with base URL, various security headers (`X-Content-Type-Options`, `Referrer-Policy`, `X-XSS-Protection`, `Content-Security-Policy`, `Strict-Transport-Security` in production).
        *   Optionally handling `withCredentials` and `responseType`.
        *   Managing a loader state (`loaderEvent`).
        *   Including `mode` parameter for non-internal users.
        *   Handling both single and multiple file uploads with `FormData`.
        *   Implementing error handling for `401 Unauthorized` responses, clearing `localStorage.removeItem('token')`, and redirecting to `/login` after storing the current path in `sessionStorage` (unless certain paths are matched). The error return for 401 is `{ ...error, error: true }`.
    *   It also exports an `async function logUserEvent` which is responsible for collecting user and merchant data, generating a session ID (using `nanoid` and `setCookie` if not present), and parsing `userAgentString` to extract OS, device, browser details. This function's implementation appears truncated in the provided log, ending mid-`parseUserAgent` function.
*   **Minor Adjustment (22/11/2025, 17:39:07):**
    *   A subtle change was introduced within the `makeAPICall`'s error handling for `401 Unauthorized` status. The error return was modified from `{ ...error, error: true }` to `{ error: true, message: error?.response?.data?.message || error.message }`.
*   **Reversion (22/11/2025, 17:39:40):**
    *   The change made at 17:39:07 was reverted. The `makeAPICall`'s 401 error handling was changed back to its original state: `return { ...error, error: true }`.

**Timestamps of Significant Changes:**

*   **22/11/2025, 17:38:55:** The initial and most comprehensive version of the `common.ts` file, establishing core API and event logging functionalities.
*   **22/11/2025, 17:39:07:** A specific modification to the error message returned on a 401 API error.
*   **22/11/2025, 17:39:40:** A quick reversion of the error message change, indicating an immediate rollback or correction.

**Patterns and Recurring Elements in Content:**

*   **Core Utility Functions:** The file is designed to host common utility functions, specifically `makeAPICall` for network requests and `logUserEvent` for client-side event tracking.
*   **Security Headers:** The `makeAPICall` consistently applies a robust set of security headers (`X-Content-Type-Options`, `Referrer-Policy`, `X-XSS-Protection`, `Content-Security-Policy`, `Strict-Transport-Security`).
*   **Loader Management:** There's a consistent pattern of invoking `loaderEvent({ count: 1 })` before an API call and `loaderEvent({ count: -1 })` after its completion or error, indicating a global loading indicator mechanism.
*   **User Role-Based Restrictions:** The `makeAPICall` function includes logic to prevent "internal users" from performing non-GET operations on specific URLs, demonstrating access control at the client-side utility layer.
*   **401 Unauthorized Handling:** The logic for handling 401 errors, involving session storage of the current path, token removal, and redirection to `/login`, is a recurring and critical piece of the authentication flow.
*   **Session ID Generation:** The `logUserEvent` function reliably generates a `userSessionId` using `nanoid` and stores it in a cookie if one doesn't exist, ensuring consistent session tracking.
*   **Truncated `logUserEvent`:** A notable pattern across all entries is that the `logUserEvent` function is consistently incomplete, ending abruptly in the `parseUserAgent` function.

## 17:34:53
The provided logs detail recent modifications across several files in the `dashboard-ui-configuration` and `checkout-dashboard-ui` repositories, focusing on configuration, shipping settings, product search, and validation logic.

### File-Specific Updates:

#### `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-configuration/src/index.ejs`
- **Timestamp**: The file was modified multiple times on `20/11/2025` at `18:37:48` and then on `21/11/2025` at `12:27:47`, `24/11/2025` at `12:17:06`, `12:17:41`, `16:50:58`, and `16:53:39`.
- **Content**: These changes consistently update the `systemjs-importmap` section within the HTML EJS template. Specifically, the URLs for `@gokwik/utilities`, `@gokwik/dashboard`, `@gokwik/payment`, `@gokwik/kwikpass`, `@gokwik/rto`, `@gokwik/kwikship`, `@gokwik/ellora`, `@gokwik/checkout-platform`, and `@gokwik/shopify-app` are being adjusted.
    - In the first change (`20/11/2025, 18:37:48`), for `isLocal` environment, `@gokwik/checkout` points to `//localhost:9002` while other `gokwik` modules point to `https://qa-dashboard.dev.gokwik.in`. The `dev`, `qa`, and `sandbox` environments all point to their respective subdomains (`dev-dashboard`, `qa-dashboard`, `sandbox-dashboard`). A new `@gokwik/kwikcomm` entry is added for the `dev` and `qa` environments.
    - Subsequent changes (`21/11/2025, 12:27:47` and `24/11/2025, 12:17:06`, `12:17:41`, `16:53:39`) reflect a shifting of the base URL for some modules in the `isLocal` environment. For example, `@gokwik/utilities`, `@gokwik/dashboard`, `@gokwik/checkout-platform`, and `@gokwik/shopify-app` are modified to point to `https://sandbox-dashboard.dev.gokwik.in/` or `https://qa-dashboard.dev.gokwik.in/` from `qa-dashboard.dev.gokwik.in/` or `dev-dashboard.dev.gokwik.in/` for `isLocal` and `dev` environments, indicating environment-specific URL adjustments, possibly for testing or deployment variations. One notable change on `24/11/2025, 16:50:58` sets `@gokwik/utilities` and `@gokwik/dashboard` to `https://dev-dashboard.dev.gokwik.in/` for the `isLocal` environment, overriding previous settings for `isLocal`. The last change on `24/11/2025, 16:53:39` reverts `@gokwik/utilities` and `@gokwik/dashboard` back to `https://sandbox-dashboard.dev.gokwik.in/` for the `isLocal` environment.
    - The New Relic browser monitoring script is consistently present in the `production` environment block.

#### `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/shipping/shippingBasedData.tsx`
- **Timestamp**: This file was modified on `20/11/2025` at `18:44:34`, `18:44:55`, and `18:49:44`, with a final change on `18:49:50`.
- **Content**:
    - The initial two changes (`18:44:34`, `18:44:55`) primarily involve a minor change in the `inputProps` for the 'Rule price' field. It changes `type: 'inputNumber'` to `type: 'input'` in the first instance, then back to `type: 'inputNumber'` for the 'Rule price' field, which is later reverted to `type: 'input'` with an `onKeyPress` validator (`20/11/2025, 18:49:44`).
    - The update on `20/11/2025, 18:49:44` adds an `onKeyPress` handler to the 'Rule price' input field. This handler prevents non-numeric or non-decimal input, ensuring only `[0-9.]` characters are accepted.
    - The final change (`18:49:50`) appears to be a re-save or minor formatting adjustment as the content is identical to the preceding entry.
- **Functionality**: This component is responsible for managing product-based shipping rules, including adding and removing products/collections, handling pricing, and validation.

#### `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/discounts/productSearch.tsx`
- **Timestamp**: Modified once on `21/11/2025, 11:48:56`.
- **Content**: This update appears to be a re-save or minor formatting change as the content is identical to the one before. It outlines a `ProductSearch` component used for adding products or collections to a discount rule, with features like search, pagination, quantity management, and filtering by agent-led tags.

#### `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/utilities/helpers/helper.ts`
- **Timestamp**: Modified on `21/11/2025` at `11:50:18` and `11:51:40`.
- **Content**: These two entries appear identical. This file provides various utility functions:
    - `getDynamicSearchFilters`: Prepares search filters for the UI.
    - `formatDateString`: Formats date strings.
    - `setCurrencySymbol`, `formatAmount`: Manages and formats currency values.
    - `shopifyAppCheckoutEnabledStatus`: Checks Shopify app checkout status based on login date.
    - `deepCompare`: Compares two objects for deep equality.
    - `debounce`: Utility for debouncing functions.
    ...
    - `extractTitlesAndDataIndexes`: Extracts table column titles and data indexes.
    - `sortDataByKey`: Sorts data based on a given key path.
    - `uploadCohortCsv`: Handles CSV file uploads for cohorts, including validation for phone numbers or emails.
    - `getFutureDateString`: Calculates a future date.
    - `compareAndCaptureArrayFields`: Compares two arrays and identifies added/removed items.
    - `areBothRangesMoreThanOneDay`: Checks if date ranges span more than one day.
    - `extractUniqueCKeysForAntColumn`: Extracts unique keys for Ant Design table columns.
    - `disabledDate`, `disabledTime`: More date/time disabling logic.
...
- **Timestamp**: Modified on `24/11/2025` at `12:31:55` and `12:36:01`.
- **Content**: These changes are identical. This component (`ShippingMethodConfig`) is responsible for configuring shipping methods, including defining shipping names, prices, and min/max order values. It includes functionality for:
    - Form management with Ant Design.
    - State management for switch toggles, modals, and edit/update status.
    - Integration with customer segments.
    - API calls for fetching and uploading data (e.g., cohort CSV files).
    - Validation rules for shipping name uniqueness and number formats for price, min/max order values.
    - Advanced conditions and dynamic field rendering.
    - File upload handling for pincode CSVs, with validation for correct format and data type (6-digit numbers).
...
- **Timestamp**: This file was extensively modified on `24/11/2025` between `15:16:01` and `17:24:39`, with numerous entries.
- **Content**: This component manages weight-based shipping methods. The series of changes indicates an active development phase focused on refining validation logic.
    - Initial changes on `15:16:01` and `15:23:28` adjust the `validateWeightRangeDifference` function call to pass an additional `isEditEnabled` boolean, indicating that validation logic for weight ranges is now context-aware (whether editing an existing entry or adding a new one).
    - The change on `17:07:59` further refines the validation by separating the calls for `minValidate` and `maxValidate` when using `validateWeightRangeDifference`, suggesting a more granular validation approach for the minimum and maximum weight values.
    - The validation rules for `shipping_name`, `min_weight`, `max_weight`, and `price` ensure that fields are not empty, contain valid positive numbers (for weights and prices), and shipping names are unique.
    - UI elements for adding, editing, and deleting shipping rules are present, along with a dropdown for actions.
    - Filtering options are available for `shipping_uuid` in the table display.

#### `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/utilities/helpers/validations.ts`
- **Timestamp**: This file was heavily modified on `24/11/2025` between `12:45:40` and `17:32:21`, with many identical changes and minor alterations.
- **Content**: This file centralizes validation logic for various discount types and shipping rates.
    - The repeated identical content for `validateTieredDiscount`, `validateDynamicDiscount`, `validateBuyXatYDiscount`, `validateDiscountCapping`, and `validateGWP` suggests that the file was frequently being saved without functional changes, or changes were being reverted/reapplied.
    - Key validations include:
        - **Tiered Discounts**: Checks for valid payment methods, unique discount codes, valid lower and upper limits (preventing overlap and ensuring `upper_limit > lower_limit`), requiring discount value for "non freebie" types, and requiring free gifts for "freebie" types. It also prevents selecting different freebie products for tiers with common ranges.
        - **Dynamic Discounts**: Validates discount names, categories (value/volume), types, and values. It checks for duplicate item volumes for "volume" category discounts and overlapping order value ranges for "value" category discounts. It ensures that only one tier can have an empty max order value and that new ranges do not overlap.
        - **Buy X at Y Discounts**: Validates discount names, match types, product quantities, and capped values, ensuring no similar discounts already exist.
        - **Discount Capping**: Validates match types, codes/regex, and capping limits.
        - **GWP (Gift With Purchase)**: Validates eligibility rules and discount codes, checking for existing similar discounts.
    - A recurring `rangesOverlap` helper function is used across multiple validation functions.
    - The updates in this file are mostly re-saves, with the underlying validation logic remaining consistent across these timestamps.

### Timestamps of Significant Changes:

- **20/11/2025, 18:37:48**: Initial broad configuration update in `index.ejs`, including adding `kwikcomm` for `dev` and `qa`.
- **20/11/2025, 18:49:44**: Refinement of input validation for 'Rule price' in `shippingBasedData.tsx` to accept only numeric/decimal inputs.
- **21/11/2025, 11:48:56**: Re-save of `productSearch.tsx`.
- **21/11/2025, 11:50:18**: First recorded change in `helper.ts`, introducing a suite of utility functions.
- **24/11/2025, 12:17:00 - 16:53:29**: Multiple environment variable changes in `.env` files, switching between `sandbox`, `qa`, and `dev` environments, and accompanying `index.ejs` updates reflecting these changes in module import URLs. This indicates active environment setup or testing.
- **24/11/2025, 12:31:55**: First significant change in `shippingBasedMethods.tsx`, introducing detailed shipping configuration logic.
- **24/11/2025, 15:16:01 - 17:24:39**: Extensive and frequent updates to `weightShipping.tsx` and `validations.ts`. This indicates a focused development effort on refining weight-based shipping logic and its associated validation rules. Specifically, the change at `17:07:59` in `weightShipping.tsx` introduced more granular validation for min/max weight ranges.

### Patterns and Recurring Elements:

- **Environment-Specific Configuration**: The `index.ejs` file consistently shows `systemjs-importmap` configurations that vary based on the `isLocal`, `dev`, `qa`, `sandbox`, and `production` environments. This pattern suggests a micro-frontend architecture where different modules (`@gokwik/*`) are loaded from different domains/localhosts based on the deployment environment.
- **Form-Based UI and Validation**: Many of the `.tsx` files (`shippingBasedData.tsx`, `shippingBasedMethods.tsx`, `weightShipping.tsx`) involve extensive use of forms (likely Ant Design's `Form` component) with associated validation rules.
- **Validation Logic**: The `validations.ts` file is a central hub for various validation functions, especially for discounts and shipping. A common `rangesOverlap` function is used to check for overlapping ranges in different contexts (tiered discounts, dynamic discounts, weight-based shipping).
- **Repetitive Saves**: There are numerous instances where the exact same code content is logged for consecutive timestamps, especially for `validations.ts`. This could indicate frequent auto-saves, minor non-functional changes (e.g., whitespace), or issues with the logging mechanism.
- **Error Handling and User Feedback**: The use of `message.error()` (from `gokwik-ui-kit`) is prevalent in validation logic across the shipping and discount components, providing immediate feedback to the user about input errors.
- **Shared Utilities**: The `helper.ts` file acts as a central repository for general utility functions (date formatting, currency formatting, deep comparison, debouncing, file uploads), indicating a well-structured codebase for common tasks.
- **Shipping Configuration Complexity**: Both `shippingBasedMethods.tsx` and `weightShipping.tsx` demonstrate a complex configuration system for shipping, involving multiple rules, advanced conditions, and external data (customer segments, pincode lists).

## 18:34:20
The provided log details changes to a single file, `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-utilities/src/services/common.ts`. This file serves as a utility hub for the dashboard application, primarily handling API calls and user event logging.

The file contains two main asynchronous functions:

1.  **`makeAPICall`**: This function is responsible for making standardized API requests using Axios.
    *   It creates an Axios instance with a base URL from `process.env.REACT_APP_BASE_URL` and applies a consistent set of security headers (`X-Content-Type-Options`, `Referrer-Policy`, `X-XSS-Protection`, `Content-Security-Policy`, and `Strict-Transport-Security` in non-development environments).
    *   It manages a loading state by calling `loaderEvent` before and after API requests.
    *   It includes logic to prevent "internal users" from performing non-GET operations on a predefined list of sensitive URLs.
    *   It supports file uploads, differentiating between single and multiple file scenarios with custom field names.
    *   Error handling specifically addresses 401 Unauthorized responses, clearing the local storage token and redirecting the user to the login page, while also preserving the current path for redirection after login.
    *   **Timestamp-specific updates for `makeAPICall`**:
        *   At `22/11/2025, 17:39:07`, the generic error handling within the `catch` block was updated. Instead of returning `{ ...error, error: true }`, it was modified to return `{ error: true, message: error?.response?.data?.message || error.message }`, aiming to provide a more specific error message.
        *   However, at `22/11/2025, 17:39:40`, this specific change was reverted, and the error return structure went back to `{ ...error, error: true }`.

2.  **`logUserEvent`**: This function is designed to capture and process user interaction events.
    *   It gathers common parameters such as `tracking_id`, `request_id`, `email`, `session_id` (generated using `nanoid` and stored in a cookie if not present), `merchant_id`, `merchant_name`, `username`, `role`, and `landing_page`.
    *   It includes a `parseUserAgent` helper function to extract details like operating system, device, OS version, browser, browser version, and whether the user is in a webview (`is_wv`) from the `navigator.userAgent` string.

**Patterns and Recurring Elements:**

*   **State and Storage Management**: The file extensively uses `dashboardStore` to access user and merchant details. It also frequently interacts with `localStorage` and `sessionStorage` for tokens, user mode, merchant IDs, and redirection paths. Cookies are used for session ID management.
*   **Environment-Specific Logic**: `process.env.REACT_APP_BASE_URL` and `process.env.NODE_ENV` are used to configure the Axios instance and security headers based on the deployment environment.
*   **Security Headers**: A consistent set of HTTP security headers is applied to all API requests to enhance application security.
*   **Loading State Feedback**: The `loaderEvent` utility is consistently used to signal loading states to the user.
*   **URL Path Analysis**: There's a recurring pattern of checking `window.location.pathname` for conditional logic, such as determining if a redirect to login is necessary or deriving the `landing_page` for events.

## 18:35:02
The log details development activities across different parts of a dashboard application, primarily focusing on shipping and discount configurations, as well as core utility functions.

**File-Specific Updates:**

*   `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/shipping/shippingBasedData.tsx`
    *   **Timestamp:** 20/11/2025, 18:49:44 and 18:49:50.
    *   This file defines a React component (`processProductBasedShippingData`) for managing product-based shipping rules. It includes state management for `itemsData` (products/collections) and `previousShippingData`. Key features involve mapping product and collection data for display, defining dropdown options for existing shipping IDs, and creating table column data with actions like deleting selected items. A form structure is laid out for defining shipping rules, including rule name, price, calculation method, price multiplier, and the ability to link to products/collections using a `ProductSearch` component. Validation ensures unique product variants/collections.

*   `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/discounts/productSearch.tsx`
    *   **Timestamp:** 21/11/2025, 11:48:56.
    *   This `ProductSearch` component facilitates searching and adding products or collections. It supports debounced search, pagination via `useIntersectionObserver`, and handles item quantities. It maps selected products (including variants) or collections for addition, with specific logic for gift pricing and collection quantity validation. Filtering based on `isAgentLed` flag is also present.

*   `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/utilities/helpers/helper.ts`
    *   **Timestamp:** 21/11/2025, 11:50:18 and 11:51:40.
    *   This utility file houses a collection of helper functions. These include:
        *   Dynamic search filter creation (`getDynamicSearchFilters`).
        *   Date and amount formatting (`formatDateString`, `formatAmount`), and currency symbol management (`setCurrencySymbol`).
        *   Status checking for Shopify app checkout (`shopifyAppCheckoutEnabledStatus`).
        *   Deep object comparison (`deepCompare`) with special handling for 'pincodes' arrays.
        *   Debouncing function calls (`debounce`).
        *   Date and time disabling logic for UI components (`disabledBeforeDate`, `disabledBeforeTime`, `disabledDate`, `disabledTime`).
        *   Text formatting (`capitalizeFirstLetter`, `formatNumber`).
        *   Table column data extraction (`extractTitlesAndDataIndexes`) and sorting (`sortDataByKey`).
        *   Cohort CSV file upload functionality (`uploadCohortCsv`), which validates if the file contains either all phone numbers or all emails and handles errors for empty or mixed-content files.
        *   Future date calculation (`getFutureDateString`).
        *   Comparison of array fields to identify added or removed items (`compareAndCaptureArrayFields`).
        *   Validation for date ranges (`areBothRangesMoreThanOneDay`).
        *   Data transformation for Ant Design table columns (`extractUniqueCKeysForAntColumn`).

*   `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-configuration/src/index.ejs`
    *   **Timestamp:** 21/11/2025, 12:27:47, 24/11/2025, 12:17:06, 24/11/2025, 12:17:41, 24/11/2025, 16:50:58, 24/11/2025, 16:52:06, and 24/11/2025, 16:53:39.
    *   This EJS template defines the base HTML structure for the dashboard, including meta tags, CSP, and scripts for `regenerator-runtime` and `single-spa`. The primary changes across timestamps involve adjusting the SystemJS import maps for various `@gokwik` micro-frontend modules (e.g., `configuration`, `utilities`, `dashboard`, `checkout`, `payment`, `kwikpass`, `rto`, `kwikship`, `ellora`, `checkout-platform`, `shopify-app`, `kwikcomm`). These URLs are conditionally set based on the `deployedEnv` (local, dev, qa, sandbox), indicating frequent environment configuration updates or testing. For instance, the local environment often points to `localhost` for some modules and `dev-dashboard.dev.gokwik.in` or `qa-dashboard.dev.gokwik.in` for others, with slight variations in the base URL used for `utilities` and `dashboard` modules between the different updates on 24/11/2025.

*   `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/shipping-settings/shipping-component/shippingBasedMethods.tsx`
    *   **Timestamp:** 24/11/2025, 12:31:55 and 12:36:01.
    *   This React component (`ShippingMethodConfig`) is designed for configuring general shipping methods. It uses Ant Design form elements for input fields such as 'Shipping Name', 'Shipping Price', 'Min Order Value', and 'Max Order Value'. It also includes functionality for managing "Advanced Conditions" and handling CSV uploads for pincode data, validating that uploaded files contain 6-digit pincodes or newline-separated values. Utility functions handle fetching customer segments, requesting signed URLs for AWS S3 uploads, and downloading CSV files. The component tracks edit mode, modal visibility, and file states.

*   `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/utilities/helpers/validations.ts`
    *   **Timestamp:** Numerous timestamps throughout 24/11/2025, from 12:45:40 to 17:34:51.
    *   This file contains a comprehensive suite of validation functions for various discount and shipping configurations:
        *   `rangesOverlap`: A core utility to check for overlapping numerical ranges, used widely across other validation functions.
        *   `validateTieredDiscount`: Ensures tiered discounts have a selected payment method, unique codes, valid and non-overlapping lower/upper limits, and correct freebie product assignments. It also enforces rules for discounts with empty upper limits.
        *   `validateDynamicDiscount`: Validates dynamic discounts for name validity, category consistency (order value vs. product quantity), type and value (including percentage caps), item volume, and order value range logic (min/max price, no overlaps, discount value vs. order value for fixed discounts).
        *   `validateBuyXatYDiscount`: Checks "Buy X at Y" discounts for name, match type, product quantity, capped value, and prevents similar existing discounts.
        *   `validateDiscountCapping`: Validates discount capping for match type, presence of code or regex, and capping limit.
        *   `validateGWP`: Validates Gift with Purchase (GWP) rules for eligibility match type and discount code, preventing similar existing discounts.
    *   **Pattern:** Many entries for this file show identical or near-identical content, suggesting frequent saves without significant functional changes between specific logs. However, an observable pattern in the call to `validateWeightRangeDifference` within `weightShipping.tsx` indicates an evolution of validation logic, with additional parameters like `isEditEnabled` being introduced and a later refactoring into separate min/max checks.

*   `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/shipping-settings/shipping-component/weightShipping.tsx`
    *   **Timestamp:** Numerous timestamps throughout 24/11/2025, from 15:16:01 to 17:36:11.
    *   This component (`WeightBaseShippingMethods`) manages weight-based shipping rules. It uses an Ant Design form and table structure. The form fields include 'Shipping Name' (with uniqueness and alphanumeric validation), 'Shipping ID' (with a tooltip, search, and "All" option), 'Min Weight (KG)', 'Max Weight (KG)', and 'Shipping Price' (must be positive and non-zero). It includes functionality for adding new rules, editing existing ones, and deleting. The component dynamically filters table data by shipping UUIDs.
    *   **Pattern:** Updates indicate an iterative refinement of the form's submission and validation logic. Specifically, the call to `validateWeightRangeDifference` from `validations.ts` undergoes changes: initially taking `weightConfig` directly, then being modified to pass `isEditEnabled` (around 15:23:28), and later being split into `minValidate` and `maxValidate` calls for more granular error handling (around 17:20:50). The `weightConfig` parameter in `validateWeightRangeDifference` was also adjusted to specifically pass `weightConfig?.weight_based_shipping_rates` (around 17:07:59). These changes highlight ongoing efforts to improve validation accuracy and user feedback for weight-based shipping rules.

**Recurring Elements and Patterns:**

*   **Ant Design (gokwik-ui-kit):** Components from `gokwik-ui-kit` (Button, Form, Input, Table, Dropdown, Tooltip, message, etc.) are consistently used across multiple UI-related files, indicating a standardized UI framework.
*   **Form Management and Validation:** There's a strong recurring theme of form creation, state management (`useState`, `useMemo`, `Form.useForm`, `Form.useWatch`), and complex validation logic, especially for various shipping and discount rules. The `rangesOverlap` function is a central utility for range-based validations.
*   **Micro-frontend Architecture:** The `index.ejs` file demonstrates an active micro-frontend architecture with dynamic loading of modules based on the deployment environment, requiring frequent updates to import maps.
*   **API Interaction:** Components frequently interact with backend APIs for data fetching and persistence, including operations for uploading and downloading CSV files with presigned URLs.
*   **Detailed Error Messaging:** Validation functions consistently return objects with `msg` and `status` properties, providing specific error messages to the user.
*   **Focus on Dashboard Configuration:** A significant portion of the changes focuses on administrative settings within a dashboard context, specifically around shipping methods (product-based and weight-based) and various discount types.

## 19:47:56
The log details changes made to a single file: `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-utilities/src/services/common.ts`. The updates occurred within a short timeframe on **22/11/2025**.

**File-specific updates for `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-utilities/src/services/common.ts`:**

*   **22/11/2025, 17:38:55 (Initial state)**:
    *   The file contains two main asynchronous functions: `makeAPICall` and `logUserEvent`.
    *   `makeAPICall` is designed for making authenticated Axios API requests. Key features include:
        *   Restricting non-GET operations for "internal users" on a defined list of URLs.
        *   Setting up Axios instances with security headers (e.g., `X-Content-Type-Options`, `Referrer-Policy`, `X-XSS-Protection`, `Content-Security-Policy`, `Strict-Transport-Security` for non-development environments).
        *   Handling `withCredentials` based on `skipCookies`.
        *   Managing loading states via `loaderEvent`.
        *   Supporting both single and multiple file uploads using `FormData`.
        *   Implementing comprehensive error handling, specifically for `401 Unauthorized` responses. Upon a 401, it clears the `localStorage` token, sets a `redirectTo` path in `sessionStorage` (unless on specific login/signup routes), and navigates to `/login`. The error return for 401 is structured as `{ ...error, error: true }`.
    *   `logUserEvent` is responsible for logging user interaction events. It includes:
        *   A `getFixedParams` helper to gather consistent parameters for events, such as `tracking_id`, user email, a generated `userSessionId` (stored in a cookie if new), merchant details (`m_id`, `short_name`), username, role, current landing page, and a timestamp.
        *   A `parseUserAgent` helper function to extract details like OS, device, browser, and their versions from the `navigator.userAgent` string, indicating whether the user is in a webview (`is_wv`).

*   **22/11/2025, 17:39:07 (First modification)**:
    *   A specific change was made within the `catch` block of the `makeAPICall` function.
    *   The return statement for the `401 Unauthorized` error condition was modified. It changed from returning the full error object (`{ ...error, error: true }`) to a more standardized error message format: `{ error: true, message: error?.response?.data?.message || error.message }`. This aimed to provide a cleaner, more consistent error response.

*   **22/11/2025, 17:39:40 (Second modification/Reversion)**:
    *   This update reverts the change made in the previous timestamp.
    *   The error handling for the `401 Unauthorized` status code within `makeAPICall` was changed back to its original state: `return { ...error, error: true }`. This effectively undoes the standardization of the 401 error message.

**Patterns and Recurring Elements:**

*   All recorded changes target the same file, `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-utilities/src/services/common.ts`, within a very short period (less than a minute). This suggests focused, rapid iteration or a quick test/revert cycle.
*   The primary focus of the changes is on the `makeAPICall` function's error handling for `401 Unauthorized` responses, indicating a specific review or adjustment related to how these authentication errors are reported or processed.
*   The core functionality of both `makeAPICall` (API interaction, security, loading) and `logUserEvent` (analytics, user agent parsing) remains consistent across all logged states, with only the one minor detail in error returns changing.

## 20:47:50
The provided logs detail changes to a single file: `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-utilities/src/services/common.ts`.

**File-Specific Updates:**

*   **`/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-utilities/src/services/common.ts`**:
    *   This file contains core utility functions for API calls (`makeAPICall`) and user event logging (`logUserEvent`).
    *   The `makeAPICall` function handles Axios requests, including file uploads, loader events, and unauthorized (401) error redirection to the login page.
    *   The `logUserEvent` function captures user agent details, session information, and merchant data to prepare structured event logs.

**Timestamps of Significant Changes:**

*   **22/11/2025, 17:39:07**: A significant update occurred in the `makeAPICall` function's error handling for HTTP 401 (unauthorized) responses. The return statement within the `catch` block was modified from `{ ...error, error: true }` to `{ error: true, message: error?.response?.data?.message || error.message }`. This change aimed to provide a more descriptive error message when an unauthorized response is received.
*   **22/11/2025, 17:39:40**: The previous change to the 401 error handling in `makeAPICall` was reverted. The return statement was changed back to its original form: `{ ...error, error: true }`.

**Patterns or Recurring Elements:**

*   The primary pattern observed is a quick iteration and subsequent rollback of a specific change related to API error handling. Within less than a minute, a refinement to how unauthorized API call errors are reported was introduced and then undone, suggesting either a testing phase, a short-lived bug fix attempt, or a change of mind regarding the error structure.
*   The overall structure and logic of the `makeAPICall` and `logUserEvent` functions, including their interfaces, dependencies, and internal logic (like file upload handling, cookie management, user agent parsing), remained consistent throughout the logged changes, with the exception of the mentioned error handling modification.