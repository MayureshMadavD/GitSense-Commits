...
## 14:56:00
The file `/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/customizeUI/brandPage.tsx`, last modified on **16/11/2025, 13:10:34**, outlines a React component responsible for managing a merchant's brand settings within a checkout UI dashboard.

Key updates and features in this file include:

*   **Brand Logo Management**:
    *   **Upload Functionality**: Implements a logo upload feature allowing merchants to upload JPEG, PNG, or SVG images (under 2MB).
    *   **Image Processing**: Utilizes `cleanImageWithCanvas` for image pre-processing and `getBase64` for preview generation.
    *   **AWS S3 Integration**: Handles image storage by fetching pre-signed URLs via `APIEndPoints.presingedLogoUrl` and uploading the cleaned image directly to AWS S3 using `makeAWSPutCall`.
    *   **Persistence**: The uploaded logo path is saved to the merchant's configuration via `APIEndPoints.updateMerchant` and updated in the Redux store using `updatedLogoData`.
    *   **Display**: Shows a preview of the uploaded logo with a fallback to `noImage.svg` if no logo is present.
    *   **Validation**: Includes client-side validation for file type and size before upload.
*   **Primary Color Customization**:
    *   Integrates a `ColorPicketComponent` to allow users to select a primary theme color, which is then saved as part of the `custom_config.cssRules.primaryColor`.
*   **Configuration Management**:
    *   Leverages a `useMerchantConfigApi` custom hook to fetch and save `custom_config` data for the merchant.
    *   Initializes form fields with existing logo data and primary color from the merchant configuration upon component load.
*   **State Management and Data Flow**:
    *   Uses `react` hooks (`useEffect`, `useState`) for component lifecycle and local state.
    *   Employs `gokwik-ui-kit`'s `Form` component for form management, including `Form.useWatch` to track changes and `Form.setFieldValue` for programmatic updates.
    *   Integrates Redux (`useSelector`, `useAppDispatch`) to retrieve merchant details (`getMerchantDetails`) and dispatch brand data updates (`updateBrandData`).
    *   Updates to form values automatically trigger a change in the `isVisible` state, enabling/disabling the "Save Changes" button.
*   **API Interactions**:
    *   Centralizes API calls through `makeAPICall` and `makeAWSPutCall` utilities, using `APIEndPoints` constants and `process.env.REACT_APP_BASE_URL` for endpoint configuration.
    *   Specific API calls are made for downloading existing logos, getting pre-signed S3 URLs, and updating merchant details.
*   **UI/UX**:
    *   Structured using `gokwik-ui-kit`'s `Card`, `Row`, `Col`, `Button`, and `Upload` components for a consistent UI.
    *   Provides an `InfoComponent` with a link to a documentation page, offering users additional context.
    *   The "Save Changes" button is disabled by default and enabled only when changes are detected in the form.

**Patterns/Recurring Elements**:

*   **Gokwik UI Kit**: Extensive use of `gokwik-ui-kit` components for building the user interface, ensuring consistency across the application.
*   **Redux for Global State**: Redux is consistently used to manage global state like merchant details and to dispatch updates.
*   **Custom Hooks**: Custom hooks (`useMerchantConfigApi`, `useAppDispatch`) abstract away complex logic and Redux interactions.
*   **Centralized API Utilities**: The use of `makeAPICall` and `makeAWSPutCall` with `APIEndPoints` for all network requests.
*   **Environment Variables for API Base URL**: `process.env.REACT_APP_BASE_URL` is used to construct API endpoints, indicating a flexible deployment strategy.
*   **Form-driven UI**: The component is heavily driven by a `Form` structure, managing input values, validations, and submission logic.