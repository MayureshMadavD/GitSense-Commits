...
## 15:06:13
The file `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-utilities/src/services/common.ts` serves as a central utility for handling API requests and logging user events within a dashboard application.

The file's state at **22/11/2025, 17:38:55** defines two core asynchronous functions:
*   `makeAPICall`: This function uses Axios to perform various HTTP requests (GET, POST, PUT, PATCH, DELETE), supporting file uploads (single and multiple files with custom field names). It integrates loader and timer events for UX feedback. Key features include:
    *   Enforcement of security headers (`X-Content-Type-Options`, `Referrer-Policy`, `X-XSS-Protection`, `Content-Security-Policy`, `Strict-Transport-Security`).
    *   Conditional `withCredentials` based on `skipCookies`.
    *   Restriction of non-GET operations for `internal user` roles on a predefined list of URLs.
    *   Comprehensive error handling for 401 Unauthorized responses, which triggers a logout (clearing `localStorage.removeItem('token')`) and redirects to the `/login` page, while preserving the current path for redirection if certain conditions are met. Non-401 errors return an object like `{ ...error, error: true }`.
*   `logUserEvent`: This function is designed to capture and log user interaction events. It gathers extensive contextual data:
    *   User and merchant details from `dashboardStore`.
    *   Session management, generating a `userSessionId` using `nanoid` and storing it in a cookie for 365 days if not present.
    *   Device and browser information parsed from the `navigator.userAgent` string, attempting to extract OS, device, browser, and versions (the `parseUserAgent` function is incomplete in the provided log snippet).
    *   `landing_page` determination from `window.location.pathname`.

At **22/11/2025, 17:39:07**, a specific modification was introduced within the `makeAPICall` function's `catch` block. The return statement for general API errors (those not explicitly handled as 401 Unauthorized) was changed from `{ ...error, error: true }` to `{ error: true, message: error?.response?.data?.message || error.message }`. This aimed to provide a more specific error `message` to the caller, leveraging the server's response if available, or a generic error message.

However, the entry at **22/11/2025, 17:39:40** shows a reversion or overwrite. The code returns to its original state from 17:38:55, meaning the change to include a `message` property in the error return object for non-401 errors was removed, and the function once again returns `{ ...error, error: true }`.

**Patterns and Recurring Elements:**
*   The file `common.ts` acts as a central hub for cross-cutting concerns like API communication and event tracking.
*   The use of Axios for all API interactions is consistent.
*   Security best practices are applied via various HTTP headers.
*   Consistent use of `dashboardStore` to access user and merchant data.
*   Heavy reliance on `localStorage`, `sessionStorage`, and cookies for state persistence (tokens, merchant IDs, session IDs).
*   Integration of `loaderEvent` and `timerEvent` for visual feedback during API calls.
*   `nanoid` is a recurring tool for generating unique identifiers.
*   The `parseUserAgent` function is consistently incomplete in the provided snippets.

## 23:01:47
The log details changes exclusively to the file `/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-utilities/src/services/common.ts` across three timestamps within a single minute on 22/11/2025.

The primary update, identified at 17:39:07, occurred within the `makeAPICall` function. Specifically, the error handling for an HTTP 401 (Unauthorized) status code was modified. In the initial version (17:38:55), when a 401 error was caught, the function returned an object spreading the entire `error` object and then explicitly adding `error: true`. The update refined this to `return { error: true, message: error?.response?.data?.message || error.message }`. This change ensures that the returned error object consistently includes an explicit `message` property, pulling it from the API response data or the general error message, which improves error communication to the client.

The final entry at 17:39:40 shows no further changes to the code compared to the 17:39:07 entry, suggesting either a quick save without modifications or a re-logging of the same state.

**Recurring elements and patterns in the content:**
*   The `common.ts` file serves as a central utility for API interactions and user event logging within a dashboard UI.
*   It heavily leverages `axios` for making HTTP requests, incorporating various security headers and handling for different request types (GET, POST, file uploads).
*   API calls are integrated with UI feedback mechanisms (`loaderEvent`, `timerEvent`) for managing loading states.
*   There's a specific authorization check within `makeAPICall` to prevent 'internal users' from performing non-GET operations on certain URLs.
*   Error handling for 401 unauthorized responses is robust, including clearing `localStorage.removeItem('token')`, storing the `redirectTo` path in `sessionStorage`, and redirecting to the login page via `navigateToUrl`.
*   The `logUserEvent` function demonstrates comprehensive user and device tracking, extracting information like `tracking_id`, `userSessionId`, `merchant_id`, `username`, and detailed user agent parsing to determine OS, device, browser, and their versions. It dynamically generates a `session_id` and sets a cookie if one doesn't exist.
*   Both `makeAPICall` and `logUserEvent` interact with a `dashboardStore` to retrieve user and merchant details, highlighting a centralized state management approach.
*   Code snippets for `parseUserAgent` are consistently truncated at `if (match &&`, indicating that the log might be capturing ongoing work or incomplete segments of the file.