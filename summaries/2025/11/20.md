...
## 23:52:03
The changes log provides insights into the development of a dashboard UI, primarily focusing on customization, order management, and shipping rules.
...
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/customizeUI/brandPage.tsx`**
    ...
    *   **Updates:** This file introduces a `BrandPage` component for customizing UI branding. Key features include logo upload functionality with image cleaning, presigned URL generation for AWS S3, and validation for image type and size. It also integrates a color picker for theme customization. The component uses Redux selectors for merchant details and dispatches actions to update logo data and brand-related configurations. Form changes trigger a UI visibility update for the "Save Changes" button.
...
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/discounts/productSearch.tsx`**
    ...
    *   **Updates:** A `ProductSearch` component for searching and selecting Shopify products or collections. It supports advanced features like collating product variants, showing quantities, and filtering items by tags (e.g., `no-agent` for agent-led scenarios). The component implements infinite scrolling using `useIntersectionObserver` and debouncing for search input. It manages selected items within a modal, allowing users to add, remove, increment, or set special pricing for "top-up gifts".
...
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/orders/order-details/index.tsx`**
    ...
    *   **Updates:** This file defines the `OrderDetails` page for displaying comprehensive order information. It includes features for fetching and processing order data, transactions, and refunds. Refunds are merged to consolidate multiple attempts, and their statuses are normalized. The component calculates and displays detailed price breakups (discounts, taxes, shipping, gift cards, rewards). It dynamically sets the currency symbol, masks sensitive customer information (phone, email), and determines order failure reasons. Permissions are validated for certain actions like RTO score visibility.

*   **`/Users/mayuresh.madav/Documents/dashboard/dashboard-ui-configuration/src/index.ejs`**
    *   **Timestamps:**
        *   18/11/2025, 00:45:31
        *   20/11/2025, 18:37:48
    *   **Updates:** This is the main configuration file for the Single-SPA application. It sets up Content Security Policy (CSP), imports React and Single-SPA from CDN, and dynamically loads various `@gokwik/*` modules based on the deployment environment (`isLocal`, `dev`, `qa`, `sandbox`, `production`). The initial update introduced comprehensive New Relic browser agent configuration for performance monitoring in `production`. The later update (20/11/2025, 18:37:48) adjusted the `isLocal` environment to load `@gokwik/utilities` and `@gokwik/dashboard` from the QA environment instead of sandbox, and added `@gokwik/kwikcomm` to the `qa` deployment configuration.

*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/free-gift/freeGift.tsx`**
    *   **Timestamps:**
        *   18/11/2025, 00:49:40
        *   18/11/2025, 00:50:14 (identical code)
        *   18/11/2025, 00:58:39 (identical code)
    *   **Updates:** This file introduces the `FreeGift` component for managing automatic free gift rules. Users can define rules based on cart value or quantity, specify applicability to all products, specific products, or collections. It allows selecting free gift products, defining their quantity and price, and choosing between auto or manual selection methods. The component uses `ProductSearch` for selecting products/collections and displays rules in a table with options to edit, delete, and enable/disable them.
...
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/orderConfirmationPage/collapseFields.tsx`**
    ...
    *   **Updates:** The `CustomizeUiFields` component provides a collapsible interface for customizing various sections of the order confirmation page. These sections include Brand Logo, Dynamic Banners, Trust Banner, Order Details, Customer Information, and Support & Communication. Each section offers fields for enabling/disabling, inputting text, and picking colors. A notable feature is the drag-and-drop functionality for reordering these sections, enhancing the flexibility of the UI customization.
...
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/pages/settings/orderConfirmation/index.tsx`**
    *   **Timestamps:**
        ...
        *   19/11/2025, 21:12:22
        *   19/11/2025, 21:12:29
    *   **Updates:** This is the main page for managing the Order Confirmation UI settings. It features a two-column layout: a left column for the configuration form (containing `MainSection` which includes `CustomizeUiFields`) and a right column for a live preview (`OrderConfirmationPreview`). The page includes a switch to enable a custom order confirmation page, an informational alert, and a "Save Changes" button. Multiple minor layout adjustments were made to the `flex` properties of the `Col` components, indicating active fine-tuning of the form and preview panel widths.
...
*   **`/Users/mayuresh.madav/Documents/dashboard/checkout-dashboard-ui/src/library/components/shipping/shippingBasedData.tsx`**
    *   **Timestamps:**
        ...
        *   20/11/2025, 18:44:55
        *   20/11/2025, 18:49:44
        *   20/11/2025, 18:49:50 (identical code)
    *   **Updates:** This component, `processProductBasedShippingData`, is responsible for creating and managing product-based shipping rules. It allows users to define a rule name, price, and map it to an existing shipping ID. Configuration options include shipping calculation methods (override/add onto base), price multipliers, and combining with weight-based shipping. The component integrates `ProductSearch` for adding specific products or collections to a rule, with logic to prevent duplicate entries. A specific change (20/11/2025, 18:49:44) enforced numeric input for the `price` field using an `onKeyPress` event handler, even though the field type was set to `input`.

### Patterns and Recurring Elements:

*   **React and UI Kit Usage:** All UI-related files heavily utilize React hooks (`useState`, `useEffect`, `useMemo`, `useRef`) and components from `gokwik-ui-kit` (presumably Ant Design based) for building forms, tables, buttons, switches, and layouts.
*   **Form-Centric Development:** `Form.useForm()`, `Form.useWatch`, `form.setFieldValue`, and `form.validateFields` are consistently used across multiple components (`BrandPage`, `FreeGift`, `OrderConfirmationPage`, `shippingBasedData`) for robust form state management and validation.
*   **Configuration API Integration:** The `useMerchantConfigApi` hook (providing `config`, `refetch`, `saveConfig`) is a common pattern for interacting with and saving merchant-specific settings across different customization pages.
*   **Reusability of Components:** The `ProductSearch` component is reused in both the `FreeGift` and `shippingBasedData` features, demonstrating a modular approach to UI development. Similarly, `RenderField` is used to abstract form field rendering.
*   **Modular UI Customization:** Many components are designed to offer detailed UI customization, often involving switches to enable/disable sections, input fields for text, and color pickers for branding elements.
*   **API Interaction:** `makeAPICall` and `makeAWSPutCall` utility functions are standard for backend communication, frequently using `process.env.REACT_APP_BASE_URL` for API endpoints.
*   **Timestamp-Based File Naming:** While not explicitly shown in all file paths, the consistency of timestamps suggests a potential automated or version-controlled environment where changes are tracked precisely.
*   **Layout Adjustments:** The multiple, quick changes to `Col` flex properties for the `OrderConfirmationPage` (`19/11/2025`) indicate agile iteration and fine-tuning of UI responsiveness and visual balance between content and preview.